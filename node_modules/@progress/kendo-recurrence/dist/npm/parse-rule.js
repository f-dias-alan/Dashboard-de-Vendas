...
var parseRule = function (options) {
    ...
    for (idx = 0, length = lines.length; idx < length; idx++) {
        ...
        if (line.indexOf('DTSTART') !== -1) {
            rule.start = (0, utils_1.parseDateTime)(line);
        }
        else if (line.indexOf('DTEND') !== -1) {
            rule.end = (0, utils_1.parseDateTime)(line);
        }
        else if (line.indexOf('EXDATE') !== -1) {
            rule.exceptionDates = (0, utils_1.parseDateTimeList)(line);
        }
    ...
    for (idx = 0, length = parts.length; idx < length; idx++) {
        ...
        switch (splits[0].trim().toUpperCase()) {
            case 'FREQ':
                ...
                break;
            case 'UNTIL':
                rule.until = (0, utils_1.parseISODate)(value[0]);
                break;
            case 'COUNT':
            ...
            case 'INTERVAL':
                ...
                break;
            case 'BYSECOND':
                rule.bySecond = (0, utils_1.parseArray)(value, { start: 0, end: 60 });
                break;
            case 'BYMINUTE':
                rule.byMinute = (0, utils_1.parseArray)(value, { start: 0, end: 59 });
                break;
            case 'BYHOUR':
                rule.byHour = (0, utils_1.parseArray)(value, { start: 0, end: 23 });
                break;
            case 'BYMONTHDAY':
                rule.byMonthDay = (0, utils_1.parseArray)(value, { start: -31, end: 31 });
                break;
            case 'BYYEARDAY':
                rule.byYearDay = (0, utils_1.parseArray)(value, { start: -366, end: 366 });
                break;
            case 'BYMONTH':
                rule.byMonth = (0, utils_1.parseArray)(value, { start: 1, end: 12 });
                break;
            case 'BYDAY':
                rule.byWeekDay = (0, utils_1.parseWeekDayList)(value);
                break;
            case 'BYWEEKNO':
                rule.byWeekNumber = (0, utils_1.parseArray)(value, { start: -53, end: 53 });
                break;
            case 'BYSETPOS':
                rule.bySetPosition = (0, utils_1.parseArray)(value, { start: -366, end: 366 });
                break;
            case 'WKST':
                rule.weekStart = (0, utils_1.parseWeekDay)(value[0]);
                break;
            default:
                break;
        }
    ...
    if (rule.bySetPosition && !(0, utils_1.hasModifiers)(rule)) {
        rule.bySetPosition = [];
    }
    sortDays(rule);
    return rule;

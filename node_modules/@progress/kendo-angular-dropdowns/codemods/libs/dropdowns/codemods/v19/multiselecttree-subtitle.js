/**-----------------------------------------------------------------------------------------
* Copyright © 2026 Progress Software Corporation. All rights reserved.
* Licensed under commercial license. See LICENSE.md in the project root for more information
*-------------------------------------------------------------------------------------------*/
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = default_1;
const tslib_1 = require("tslib");
const fs = tslib_1.__importStar(require("fs"));
const codemods_1 = require("@progress/kendo-angular-common/codemods");
function default_1(fileInfo, api) {
    const filePath = fileInfo.path;
    // Handle HTML files and inline templates
    const htmlResult = (0, codemods_1.htmlTransformer)(fileInfo, api, (templateContent) => (0, codemods_1.attributeNameUpdate)(templateContent, 'kendo-multiselecttree', 'subtitle', 'adaptiveSubtitle'));
    if (filePath.endsWith('.html')) {
        if (htmlResult && htmlResult !== fileInfo.source) {
            fs.writeFileSync(filePath, htmlResult, 'utf-8');
            return htmlResult;
        }
        return fileInfo.source; // Return original source if no changes
    }
    // Handle TypeScript property transformations
    const j = api.jscodeshift;
    const rootSource = j(htmlResult || fileInfo.source);
    (0, codemods_1.tsPropertyTransformer)(fileInfo.source, rootSource, j, '@progress/kendo-angular-dropdowns', 'MultiSelectTreeComponent', 'subtitle', 'adaptiveSubtitle');
    return rootSource.toSource();
}
